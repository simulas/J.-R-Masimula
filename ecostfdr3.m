...
for i = 1:N
    ...
    for j = 1:n
        ...
        % Count the number of failures (tf) and the number of repair rate (tr)
        if T1 < t
            tf1 = tf1 + 1;
            trLp1 = trLp1 + 5/c1;
            trLp2 = trLp2 + 5/c1;
    ...
    % Determine the failure rate, lamda, for each load point
    ...
    % Determine the Unavailability, U, for each load point
    ...
    % Determine Outage duration, r, for each load point
    r1 = ULp1/lamda1;
    r2 = ULp2/lamda2;
    r3 = ULp3/lamda3;
    r4 = ULp4/lamda4;
    % Determine System indices using equation 2.1-2.9
    % Get SAIFI
    ...
    % Get AENS
    AENS = ENS/d;
    % Determining Reliability cost/worth indices
    ...
    y(i,:) = [SAIFI SAIDI CAIDI ASAI ASUI ENS AENS ECOST IAER];
